...
if [[ "$1" =~ ^(diff|gradient|minus)$ ]]; then
    method=$1
    echo "Start computing each scene result using '${method}' approach from vdp SVD-entropy..."
    echo "--------------------------------------------------------------------------"
else
            ...
            for start in {0,50,100,150}; do
                for end in {50,100,150,200}; do
                    ...
                    if [ $result -gt 0 ]; then
                        ...
                        python methods/compute_${method}_entropy.py --data data/generated/vdp_entropy_data_imnorm${imnorm}_${start}_${end}.csv --norm ${norm} --std ${std} --output vdp_entropy_${method}_imnorm${imnorm}_norm${norm}_std${std}_${start}_${end}.csv
 
                        output_filename="${output_directory}/comparisons_vdp_${method}_imnorm${imnorm}_norm${norm}_std${std}_${metric}_${start}_${end}"
                        ...
                        for scene in {"A","B","C","D","E","F","G","H","I"}; do
                            ...
                            python utils/reconstruct_image_estimated.py --data data/thresholds/vdp_entropy_${method}_imnorm${imnorm}_norm${norm}_std${std}_${start}_${end}.csv --scene ${scene} --output data/images/vdp_${scene}_${method}_imnorm${imnorm}_norm${norm}_std${std}_${start}_${end}_estimated.png

                            estimated_error=$(python utils/compare_images.py --img1 ${reference_image} --img2 data/images/vdp_${scene}_${method}_imnorm${imnorm}_norm${norm}_std${std}_${start}_${end}_estimated.png --metric ${metric})
