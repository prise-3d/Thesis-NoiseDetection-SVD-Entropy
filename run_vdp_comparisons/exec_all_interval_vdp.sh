            ...
            for start in {0,50,100,150}; do
                for end in {50,100,150,200}; do
                    ...
                    if [ $result -gt 0 ]; then
                        ...
                        echo python methods/compute_${method}_entropy.py --data data/generated/vdp_entropy_data_imnorm${imnorm}_${start}_${end}.csv --norm ${norm} --std ${std} --output vdp_entropy_${method}_imnorm${imnorm}_norm${norm}_std${std}_${start}_${end}.csv
 
                        output_filename="${output_directory}/vdp_comparisons_vdp_${method}_imnorm${imnorm}_norm${norm}_std${std}_${metric}_${start}_${end}"
                        ...
                        for scene in {"A","B","C","D","E","F","G","H","I"}; do
                            ...
                            current_output_image_path=data/images/vdp_${scene}_${method}_imnorm${imnorm}_norm${norm}_std${std}_${start}_${end}_estimated.png
                            echo python utils/reconstruct_image_estimated.py --data data/thresholds/vdp_entropy_${method}_imnorm${imnorm}_norm${norm}_std${std}_${start}_${end}.csv --scene ${scene} --output ${current_output_image_path}

                            # run vdp between ref and estimated image
                            vdp_estimated_image_without_ext=data/images/vdp_vdp_${scene}_${method}_imnorm${imnorm}_norm${norm}_std${std}_${start}_${end}_estimated
